...
**The total size of the data 1.28GB with 5872 items. **
...
### Binary VGG16
On top of the VGG16 base, we add 2 hidden layers with 100 and 50 nodes respectively. Each layer uses a SeLU activation function. Our final layer is made of 2 nodes with a softmax activation function. 
We use a dropout rate of 0.2 and a $l_1$ regularization penalty of 0.001. Our optimizer is Adamax with a learning rate of 0.001. 
...
```
Based on our small test set, we can predict COVID patients in more than `r round(b16s$categorical_accuracy,4)` of the cases.
**Large test set**
```{r, echo = F, fig.align="center"}
#load model
...
```
When we test our model on the large dataset, the accuracy increases to more than `r round(b16l$categorical_accuracy,4)`.  

&nbsp;

### Binary DenseNet201
On top of the DenseNet201 base, we add 2 hidden layers with 100 and 100 nodes respectively. Each layer uses a ReLU activation function. Our final layer is made of 2 nodes with a softmax activation function. 
We use a dropout rate of 0.2 and no $l_1$ regularization penalty. Our optimizer is adamax with a learning rate of 0.001. 
...
print(bindens)
```
Based on our small test set, we can predict COVID patients in `r round(bindens$categorical_accuracy,4)` of the cases. 
&nbsp;
**Large test set**
```{r, echo = F, fig.align="center"}
...
print(bindenl)
```
When we test our model on the large dataset, the accuracy decreases to slightly below `r round(bindenl$categorical_accuracy,4)`.
&nbsp;

### Multiclass VGG16
On top of the VGG16 base, we add 2 hidden layers with 100 and 50 nodes respectively. Each layer uses a ReLU activation function. Our final layer is made of 4 nodes with a softmax activation function. 
We use neither a dropout rate nor a $l_1$ regularization penalty. Our optimizer is RMSprop with a learning rate of 0.0001. 
...
```
Based on our small test set, we can predict COVID patients in `r round(mc16s$categorical_accuracy,4)` of the cases.
&nbsp;
**Large test set**
```{r, echo = F, fig.align="center"}
...
```
When we test our model on the large dataset, the accuracy decreases to slightly above `r round(mc16l$categorical_accuracy,4)`.
